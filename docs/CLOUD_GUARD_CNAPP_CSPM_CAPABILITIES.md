...
> **Context:** This document is written for the STAR ELZ V1 deployment running on an OCI Isolated Region (air-gapped, sovereign). All capabilities described are available in OCI Isolated Regions, which deliver the same 150+ OCI services as public commercial regions. DSTA and the Singapore Ministry of Defence (MINDEF) / Singapore Armed Forces (SAF) selected Oracle Cloud Isolated Region specifically for these security capabilities in a sovereign, air-gapped environment.
...
For DSTA/MINDEF environments where workloads span OCI Isolated Region and other approved cloud environments, the Autonomous AI Lakehouse provides the data integration layer:
